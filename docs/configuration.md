...
| `RESEND_API_KEY` | API key for [Resend](https://resend.com). Required unless SMTP is configured. |
| `SMTP_HOST` | SMTP host (required if using SMTP) |
| `SMTP_USERNAME` | SMTP username (required if using SMTP) |
| `SMTP_PASSWORD` | SMTP password (required if using SMTP) |
